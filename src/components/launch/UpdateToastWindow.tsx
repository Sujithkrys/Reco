...
function getDetail(payload: UpdateToastPayload, t: Translate) {
	if (payload.phase === "available" && payload.isExperimental) {
		return t(
			...
			"You've opted into experimental updates so you have the choice to test the latest update of Reco before it's widely available.",
		);
	}
...
export function UpdateToastWindow() {
	...
	return (
		<div className={`${styles.window} launch-theme`}>
			<section className={styles.card} aria-live="polite" aria-label="Reco update">
				<div
