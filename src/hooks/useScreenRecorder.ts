...
export function useScreenRecorder(): UseScreenRecorderReturn {
	...
	const preparePermissions = useCallback(async (options: { startup?: boolean } = {}) => {
		...
		if (!screenPermission.success || screenPermission.status !== "granted") {
			...
			alert(
				options.startup
					? "Reco needs Screen Recording permission before you start. System Settings has been opened. After enabling it, quit and reopen Reco."
					: "Screen Recording permission is still missing. System Settings has been opened again. Enable it, then quit and reopen Reco before recording.",
			);
			return false;
		}
		...
		alert(
			options.startup
				? "Reco also needs Accessibility permission for cursor tracking. System Settings has been opened. After enabling it, quit and reopen Reco."
				: "Accessibility permission is still missing. System Settings has been opened again. Enable it, then quit and reopen Reco before recording.",
		);
