...
export class ModernVideoExporter {
	...
	private getLightningErrorGuidance(message: string): string[] {
		...
		if (isVideoDecodeFailure) {
			guidance.add(
				"The input video decoder failed before Reco could finish rendering the source frames.",
			);
			guidance.add(
	...
	private buildLightningExportError(error: unknown): string {
		...
		if (this.runtimeDiagnostics.appVersion) {
			lines.push(`Reco version: ${this.runtimeDiagnostics.appVersion}`);
		}
	...
	private async tryExportNativeStaticLayout(
		...
		const startedAt = this.getNowMs();
		const sessionId = `reco-static-layout-${Date.now()}-${Math.random()
			.toString(36)
			.slice(2, 8)}`;
