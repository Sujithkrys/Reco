...
const AUTO_UPDATES_DISABLED = process.env.RECO_DISABLE_AUTO_UPDATES === "1";
const UPDATE_FEED_URL_OVERRIDE = process.env.RECO_UPDATE_FEED_URL?.trim() ?? "";
const UPDATER_LOG_PATH =
	process.env.RECO_UPDATER_LOG_PATH?.trim() || path.join(USER_DATA_PATH, "updater.log");
const DEV_UPDATE_PREVIEW_VERSION = "9.9.9";
const DEV_UPDATE_PREVIEW_IS_EXPERIMENTAL =
	process.env.RECO_DEV_PREVIEW_EXPERIMENTAL_UPDATE === "1";
...
function applyExperimentalUpdatesPreference() {
	...
	// Changing channels enables downgrades inside electron-updater. Reco never
	...
	return {
		...
		detail:
			normalizedProgress >= 100
				? "Finishing the update download. Reco will restart as soon as the installer is ready."
				: remainingMb !== null
					? `${remainingMb.toFixed(1)} MB left before Reco restarts.`
					: "Downloading the update now. Reco will restart when it finishes.",
		delayMs: UPDATE_REMINDER_DELAY_MS,
		isExperimental,
...
export async function downloadAvailableUpdate(
	...
	setUpdateStatusSummary({
		status: "downloading",
		availableVersion,
		detail: `Downloading Reco ${availableVersion}`,
	});
...
async function showAvailableUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		message: `Reco ${version} is available${isExperimental ? " on the experimental channel" : ""}.`,
		detail: isPreview
...
async function showDownloadedUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		title: "Update Ready",
		message: isPreview
			? `Reco ${version} is ready to install.`
			: `Reco ${version} has been downloaded.`,
		detail: isPreview
...
async function showUpdateErrorDialog(
	...
	await showMessageBox(getMainWindow, {
		type: "error",
		title: "Update Failed",
		message: `Reco ${version} could not be downloaded.`,
		detail: String(error),
		buttons: ["OK"],
		defaultId: 0,
		noLink: true,
		...
		if (options?.manual) {
			await showMessageBox(getMainWindow, {
				...
				detail: AUTO_UPDATES_DISABLED
					? "This build disabled auto-updates through RECO_DISABLE_AUTO_UPDATES=1."
...
export function setupAutoUpdates(
	...
	autoUpdater.on("update-available", (info) => {
		...
		setUpdateStatusSummary({
			status: "available",
			availableVersion: info.version,
			detail: `Reco ${info.version} is available.`,
		});
	...
	autoUpdater.on("update-not-available", () => {
		...
		setUpdateStatusSummary({
			status: "up-to-date",
			availableVersion: null,
			detail: `Reco ${app.getVersion()} is up to date.`,
		});
		clearVisibleUpdateToast(sendToRenderer);
		manualCheckRequested = false;
	});
	...
	autoUpdater.on("update-downloaded", (info) => {
		...
		setUpdateStatusSummary({
			status: "ready",
			availableVersion: info.version,
			detail: `Reco ${info.version} is ready to install.`,
		});
		clearDeferredReminderTimer();
