...
const ffmpegCommand = resolveToolCommand(["RECO_FFMPEG_EXE"], "ffmpeg-static", "ffmpeg");
const ffprobeCommand = resolveToolCommand(["RECO_FFPROBE_EXE"], "ffprobe-static", "ffprobe");
...
function shouldRaiseChildPriority() {
	return process.env.RECO_NVIDIA_CUDA_EXPORT_HIGH_PRIORITY !== "0";
}
...
const outputPath = resolve(
	getArg("--output", join(scriptDir, "reco-nvdec-nvenc-mp4-output.mp4")),
);
...
function resolveNativeProbePath() {
	const configuredPath = process.env.RECO_NVIDIA_CUDA_EXPORT_EXE;
	const platformArch = process.arch === "arm64" ? "win32-arm64" : "win32-x64";
	const candidates = [
		configuredPath,
		join(scriptDir, "build", "Release", "reco-nvidia-cuda-compositor.exe"),
		join(
			repoRoot,
			"electron",
			"native",
			"bin",
			platformArch,
			"reco-nvidia-cuda-compositor.exe",
		),
		// Backward-compatible legacy helper path while old work dirs are being retired.
		join(scriptDir, "build", "Release", "reco-nvdec-nvenc-probe.exe"),
	].filter(Boolean);
