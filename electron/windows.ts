...
const WINDOW_ICON_FILENAME =
	process.platform === "darwin" ? "recomac-512.png" : "reco-512.png";
...
function getEditorWindowQuery(): Record<string, string> {
	...
	if (process.env.RECO_DEV_OPEN_RECORDING_INPUT) {
		query.devOpenInput = process.env.RECO_DEV_OPEN_RECORDING_INPUT;
	}
	if (process.env.RECO_DEV_OPEN_RECORDING_WEBCAM) {
		query.devOpenWebcam = process.env.RECO_DEV_OPEN_RECORDING_WEBCAM;
	}

	if (process.env.RECO_SMOKE_EXPORT === "1") {
		query.smokeExport = "1";
		if (process.env.RECO_SMOKE_EXPORT_INPUT) {
			query.smokeInput = process.env.RECO_SMOKE_EXPORT_INPUT;
		}
		if (process.env.RECO_SMOKE_EXPORT_OUTPUT) {
			query.smokeOutput = process.env.RECO_SMOKE_EXPORT_OUTPUT;
		}
		if (process.env.RECO_SMOKE_EXPORT_USE_NATIVE === "1") {
			query.smokeUseNativeExport = "1";
		}
		if (process.env.RECO_SMOKE_EXPORT_ENCODING_MODE) {
			query.smokeEncodingMode = process.env.RECO_SMOKE_EXPORT_ENCODING_MODE;
		}
		if (process.env.RECO_SMOKE_EXPORT_SHADOW_INTENSITY) {
			query.smokeShadowIntensity = process.env.RECO_SMOKE_EXPORT_SHADOW_INTENSITY;
		}
		if (process.env.RECO_SMOKE_EXPORT_WEBCAM_INPUT) {
			query.smokeWebcamInput = process.env.RECO_SMOKE_EXPORT_WEBCAM_INPUT;
		}
		if (process.env.RECO_SMOKE_EXPORT_WEBCAM_SHADOW) {
			query.smokeWebcamShadow = process.env.RECO_SMOKE_EXPORT_WEBCAM_SHADOW;
		}
		if (process.env.RECO_SMOKE_EXPORT_WEBCAM_SIZE) {
			query.smokeWebcamSize = process.env.RECO_SMOKE_EXPORT_WEBCAM_SIZE;
		}
		if (process.env.RECO_SMOKE_EXPORT_PIPELINE) {
			query.smokePipelineModel = process.env.RECO_SMOKE_EXPORT_PIPELINE;
		}
		if (process.env.RECO_SMOKE_EXPORT_BACKEND) {
			query.smokeBackendPreference = process.env.RECO_SMOKE_EXPORT_BACKEND;
		}
		if (process.env.RECO_SMOKE_EXPORT_RENDER_BACKEND) {
			query.smokeRenderBackend = process.env.RECO_SMOKE_EXPORT_RENDER_BACKEND;
		}
		if (process.env.RECO_SMOKE_EXPORT_MAX_ENCODE_QUEUE) {
			query.smokeMaxEncodeQueue = process.env.RECO_SMOKE_EXPORT_MAX_ENCODE_QUEUE;
		}
		if (process.env.RECO_SMOKE_EXPORT_MAX_DECODE_QUEUE) {
			query.smokeMaxDecodeQueue = process.env.RECO_SMOKE_EXPORT_MAX_DECODE_QUEUE;
		}
		if (process.env.RECO_SMOKE_EXPORT_MAX_PENDING_FRAMES) {
			query.smokeMaxPendingFrames = process.env.RECO_SMOKE_EXPORT_MAX_PENDING_FRAMES;
		}
		if (process.env.RECO_SMOKE_EXPORT_PROJECT) {
			query.smokeProject = process.env.RECO_SMOKE_EXPORT_PROJECT;
		}
		if (process.env.RECO_SMOKE_EXPORT_QUALITY) {
			query.smokeQuality = process.env.RECO_SMOKE_EXPORT_QUALITY;
		}
		if (process.env.RECO_SMOKE_EXPORT_FPS) {
			query.smokeFps = process.env.RECO_SMOKE_EXPORT_FPS;
		}
	}
...
export function createHudOverlayWindow(): BrowserWindow {
	...
	const win = new BrowserWindow({
		...
		resizable: false,
		alwaysOnTop: true,
		// The HUD is Reco's persistent top-level window, so it owns the
	...
	const showHudWindow = () => {
		...
		if (process.platform === "win32") {
			...
			// always-on-top HUD must not steal focus when Reco starts.
			win.showInactive();
		} else {
			win.show();
		}
...
export function createUpdateToastWindow(): BrowserWindow {
	...
	win.setVisibleOnAllWorkspaces(true, {
		visibleOnFullScreen: true,
		// Keep Reco a foreground application so macOS does not temporarily
