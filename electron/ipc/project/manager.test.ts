...
describe("local media path policy", () => {
	...
	beforeEach(async () => {
		tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "reco-media-policy-"));
	...
	it("preserves an existing project thumbnail when no replacement is provided", async () => {
		const projectPath = path.join(tempRoot, "Projects", "demo.reco");
	...
	it("loads project files that start with a UTF-8 byte order mark", async () => {
		...
		const projectPath = path.join(tempPath, "recording.reco");
		await fs.writeFile(videoPath, "test-video");
		await fs.writeFile(
			projectPath,
			`\uFEFF${JSON.stringify({
	...
	it("rejects invalid project payloads before approving media paths", async () => {
		...
		const projectPath = path.join(tempPath, "invalid.reco");
		await fs.mkdir(downloadsPath, { recursive: true });
		await fs.writeFile(videoPath, "test-video");
		await fs.writeFile(
			projectPath,
