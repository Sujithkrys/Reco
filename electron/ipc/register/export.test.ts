...
async function makeTempDir() {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "reco-export-move-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("moveExportedTempFile", () => {
	it("moves an app-managed export temp file to the selected destination", async () => {
		...
		await fs.writeFile(tempPath, "reco-export");

		await moveExportedTempFile(tempPath, destinationPath);

		await expect(fs.readFile(destinationPath, "utf8")).resolves.toBe("reco-export");
