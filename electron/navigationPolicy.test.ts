...
describe("isInternalRendererTarget", () => {
	it.each([
		...
		[
			"file:///opt/Reco/dist/index.html?windowType=editor",
			"file:///opt/Reco/dist/index.html?windowType=hud-overlay#status",
		],
	...
	it.each([
		...
		["https://reco.example/", "http://reco.example/"],
		["https://reco.example/", "https://user:pass@reco.example/"],
		["file:///opt/Reco/dist/index.html", "file:///etc/passwd"],
		["file:///opt/Reco/dist/index.html", "data:text/html,hello"],
...
describe("navigation event handlers", () => {
	...
	it("stops same-file query mutation without externalizing it", () => {
		...
		handler({
			url: "file:///opt/Reco/dist/index.html?smokeExport=1",
			preventDefault,
		});
	...
	it("does not trust a renderer-mutated URL as an exact reload", () => {
		let currentUrl = "file:///opt/Reco/dist/index.html?windowType=editor";
		const on = vi.fn();
		const webContents = {
			getURL: () => currentUrl,
			on,
		...
		currentUrl = "file:///opt/Reco/dist/index.html?windowType=source-selector";
	...
	it("trusts an exact reload after a completed document navigation", () => {
		...
		const loadedUrl = "file:///opt/Reco/dist/index.html?windowType=editor";
