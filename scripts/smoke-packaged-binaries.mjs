...
const packageName = packageJson.name ?? "reco";
...
function getExpectedNativeHelperFiles(archTag) {
	if (archTag.startsWith("win32-")) {
		...
		if (archTag === "win32-x64") {
			helpers.push({
				name: "reco-nvidia-cuda-compositor.exe",
	...
	if (archTag.startsWith("darwin-")) {
		return [
			{
				name: "reco-screencapturekit-helper",
				label: "ScreenCaptureKit helper",
				executable: true,
			},
			{ name: "reco-window-list", label: "Window list helper", executable: true },
			{ name: "reco-system-cursors", label: "System cursor helper", executable: true },
			{
				name: "reco-native-cursor-monitor",
