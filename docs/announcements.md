...
Reco can show dismissible announcements in the editor as a popup, carousel slide, lightweight live notification, or header banner. Popups can contain images or video; notifications and banners are text-only with optional buttons.
...
```json
{
	...
	"announcements": [
		{
			"id": "reco-1.4-release",
			"title": "A faster Reco is here",
			"body": "Exports are faster and cursor motion is smoother. Thanks for using Reco!",
			"presentation": "popup",
			"audience": "editor",
			"priority": 10,
			"mediaMode": "cover",
			...
			"media": {
				"type": "image",
				"url": "https://example.com/reco-1.4-banner.jpg",
				"alt": "Reco 1.4 feature preview"
			},
			"action": {
				"label": "See what changed",
				"url": "https://github.com/webadderallorg/Reco/releases"
			}
		},
		{
			"id": "reco-maintenance-notice",
...
Set `RECO_ANNOUNCEMENTS_URL` before launching the app to use a different HTTPS feed. Set it to `off` to disable remote announcements.
